...
public final class TimeCanvasUtil {
    ...
    public static void fillRect(LayoutPosition p, double hPadding, Paint fill, double radius, GraphicsContext gc) {
        CanvasUtil.fillRect(p.getX(), p.getY(), p.getWidth(), p.getHeight(), hPadding, fill, radius, gc);
    }

    public static void strokeRect(LayoutPosition p, double hPadding, Paint stroke, double radius, GraphicsContext gc) {
        CanvasUtil.strokeRect(p.getX(), p.getY(), p.getWidth(), p.getHeight(), hPadding, stroke, radius, gc);
    }

    public static void fillStrokeRect(LayoutPosition p, double hPadding, Paint fill, Paint stroke, double radius, GraphicsContext gc) {
        fillRect(p, hPadding, fill, radius, gc);
        strokeRect(p, hPadding, stroke, radius, gc);
    }

    public static void fillText(LayoutPosition p, double hPadding, String text, Paint fill, VPos baseline, TextAlignment textAlignment, GraphicsContext gc) {
        CanvasUtil.fillText(p.getX(), p.getY(), p.getWidth(), p.getHeight(), hPadding, text, fill, baseline, textAlignment, gc);
    }

    public static void fillTopCenterText(LayoutPosition p, double hPadding, String text, Paint fill, GraphicsContext gc) {
        fillText(p, hPadding, text, fill, VPos.TOP, TextAlignment.CENTER, gc);
    }

    public static void fillCenterLeftText(LayoutPosition p, double hPadding, String text, Paint fill, GraphicsContext gc) {
        fillText(p, hPadding, text, fill, VPos.CENTER, TextAlignment.LEFT, gc);
    }

    public static void fillCenterText(LayoutPosition p, double hPadding, String text, Paint fill, GraphicsContext gc) {
