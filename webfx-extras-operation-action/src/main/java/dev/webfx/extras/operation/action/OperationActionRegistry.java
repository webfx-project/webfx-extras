...
public final class OperationActionRegistry {
    ...
    private static OperationActionAuthorizer AUTHORIZER; // Injected by ModalityClientOperationActionsLoader
    ...
    // Keeping a list of operation actions whose graphical actions are not yet registered => the binding is deferred
    ...
    public ObservableBooleanValue authorizedOperationActionProperty(Object operationCode, AsyncFunction<Object, Boolean> authorizationFunction) {
        ...
        AsyncFunction<Object, Boolean> embedAuthorizationFunction = new AsyncFunction<Object, Boolean>() { // Using a lambda expression here causes a wrong GWT code factorization that leads to an application crash! Keeping the Java 7 style solves the problem.
            @Override
    ...
    // because the graphical action can change (ex: cache value on application start and then refreshed value from the database)
